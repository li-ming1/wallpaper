...
## 2026-03-30 静态阶段渲染缓存回收
- 目标：进一步降低“长时间静态/窗口遮挡/频繁换源”场景下的无效显存与进程驻留。
- 新增策略模块：`video_surface_cache_policy`。
- 接口扩展：`IWallpaperHost` 新增 `TrimMemory()`，Windows 宿主实现为“释放渲染侧视频纹理 + Flush”，不销毁交换链，因此静态最后一帧仍由 backbuffer 保持。
- pause 路径：当进入静态 pause 且持续时间超过 `trimDecodeCacheAfter` 阈值（默认 2.5s）时，除了清理解码侧缓存，也回收渲染侧视频纹理。
- 换源路径：在 `ApplyVideoPath()` 确认路径变化后，先主动 trim 当前渲染缓存，再打开新视频，降低频繁切换时的瞬时重叠驻留。
- 基准验证（本机临时配置指向仓库 `kuroha_1080p30_h264.mp4`）：
