...
### Phase 67: 静态阶段渲染缓存回收
- **Status:** complete
- **Started:** 2026-03-30 17:16:00
- **Completed:** 2026-03-30 17:45:00
- Actions taken:
  - 新增 `video_surface_cache_policy` 头/源与单测，先锁定“换源 trim”和“长静态 pause trim”条件。
  - 扩展 `IWallpaperHost` 接口，加入 `TrimMemory()`；Windows 宿主实现为释放渲染侧视频纹理并 `Flush()`。
  - 在 `App::ApplyVideoPath()` 接入换源前 render cache trim，在 pause 路径接入长静态阈值后的 render cache trim。
  - 运行真实基准：临时写入本地配置指向仓库测试视频，测量桌面动态场景与前台普通窗口静态场景。
- Verification:
  - `./scripts/run_tests.ps1 -BuildDir build_tmp/phase67_surface_cache_fix` -> pass（176/176）
  - `./scripts/build_app.ps1 -BuildDir build_tmp/phase67_surface_cache_fix_app` -> pass
  - `desktop` benchmark -> CPU avg `0.0284%` / WS max `14,786,560`
  - `foreground_notepad_pause` benchmark -> CPU avg `0.0378%` / WS max `14,761,984`
- Files created/modified:
...
### Phase 70: DXVA NV12 子资源 SRV 复用
- **Status:** complete
- **Started:** 2026-03-30 19:20:00
- Actions taken:
  - Red: 新增 `tests/gpu_nv12_srv_cache_policy_tests.cpp`，并在测试清单中接入，先触发“缺少策略头文件”编译失败。
  - Green: 新增 `gpu_nv12_srv_cache_policy`，提供“源纹理/尺寸变化才重置缓存”的判定。
  - Green: `wallpaper_host_win` 新增 GPU NV12 SRV 缓存池（按 `subresourceIndex` 复用），避免子资源切换时每帧 `CreateShaderResourceView`。
  - Green: `ReleaseVideoTexture()` 区分 CPU NV12 与 GPU NV12 缓存释放路径，防止 SRV 生命周期冲突。
  - Verification: 全量单测、构建、真实链路运行与 metrics 检查。
- Verification:
  - `./scripts/run_tests.ps1 -BuildDir build_tmp/phase72_gpu_nv12_cache_red` -> fail as expected（缺少头文件）
  - `./scripts/run_tests.ps1 -BuildDir build_tmp/phase72_gpu_nv12_cache_green` -> pass（191/191）
  - `./scripts/build_app.ps1 -BuildDir build` -> pass
  - 真实会话 `sess_1774859453779_4276`: `decode_path=dxva_zero_copy`、`effective_fps=30`、trim 后 working set 约 `18~20MB`
- Files modified:
  - include/wallpaper/gpu_nv12_srv_cache_policy.h
  - src/gpu_nv12_srv_cache_policy.cpp
  - tests/gpu_nv12_srv_cache_policy_tests.cpp
  - src/win/wallpaper_host_win.cpp
  - CMakeLists.txt
  - scripts/run_tests.ps1
  - scripts/build_app.ps1
